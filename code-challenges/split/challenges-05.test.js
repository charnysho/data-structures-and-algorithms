...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(line => {
    let arr = line.match(/\b(\w+)\b/g).slice(2);
    if(arr.length > 1) {
      let str = '';
      arr.forEach(word => {
        str = str + ' ' + word;
      });
      if(str.includes(' sized')) {
        str = str.replace(' sized', '');
      }
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  return str.splice(0, -numberOfCharacters);
};
