...
const count = (target, input) => {
  var result = 0;
  input.map(subarray => subarray.filter(x => x === target)).map(i => result = result + i.length);
  return result;
};
...
const totalSum = (input) => {
  var total = 0;
  input.map(subarray => subarray.map(x => total = total + x));
  return total;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let firstPart = input.map(subarray => subarray.filter(x => (!isNaN(x) && typeof x !== 'string' && x % 5 ===0)));
  let result = firstPart.map(arr => arr.map(y => Math.pow(2, y)));
  return result;
};
...
let findMaleAndFemale = (data) => {
  ...
  newArr.map(obj => (result === '' ? result = obj.name : result = `${result} and ${obj.name}`));
  return result;
};
...
let findShortest = (data) => {
  let smallest = Number(data[0].height);
  let smallestName = '';
  data.map(function(obj) {
    if (Number(obj.height) < smallest) {
      smallest = obj.height;
      smallestName = obj.name; console.log(smallestName);
    }
  });
  return smallestName;
};
